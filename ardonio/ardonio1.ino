...
void setup()
{
  ...
  while (!readinit)
  {
    if (Serial3.available())
    {
      ...
      currentPoint_X = initdata[0] * 256 + initdata[1];
      currentPoint_Y = initdata[2] * 256 + initdata[3];
      northAt = initdata[4] * 256 + initdata[5];
 ...
 void loop()
 {
     ...
     dis = sqrt(sqr(nextPoint_X - currentPoint_X)+ sqr(nextPoint_Y - currentPoint_Y));
...
void goStraight(double distance) {
  ...
  digitalWrite(4, 1);
  digitalWrite(7, 1);
  bspeedL = 0; // adjust l wheel speed
  bspeedR = 0;
